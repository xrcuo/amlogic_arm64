...
在 [xrcuo/kernel_arm64](https://github.com/xrcuo/xrcuo/kernel_arm64) 的 [Releases](https://github.com/xrcuo/xrcuo/kernel_arm64/releases) 中提供了预编译的内核文件。
...
| -f     | configFlavor | 指定从内核仓库 [xrcuo/kernel_arm64](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release) 下载的配置文件 `config-*` 到本地 [tools/config](tools/config) 目录。若本地已存在对应内核版本的配置文件且未设置此参数，则跳过下载。可选项为内核仓库中存在的[目录名](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release)，例如：`stable` / `rk3588` / `rk35xx`。默认值：`stable` |
...
’ 提示：推荐使用 `unifreq` 的 [linux-6.1.y](https://github.com/unifreq/linux-6.1.y)、[linux-5.15.y](https://github.com/unifreq/linux-5.15.y)、[linux-5.10.y](https://github.com/unifreq/linux-5.10.y) 等仓库的内核源代码进行编译，其中已针对相关设备添加了驱动和补丁。配置文件推荐使用 [xrcuo/kernel_arm64](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release) 中的模板，已针对支持的设备进行了预配置，可在此基础上进行个性化定制。
...
| config_flavor     | stable           | 指定从内核仓库 [xrcuo/kernel_arm64](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release) 下载的配置文件 `config-*` 到本地 [tools/config](tools/config) 目录。若本地已存在对应内核版本的配置文件且未设置此参数，则跳过下载。可选项为内核仓库中存在的[目录名](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release)，例如：`stable` / `rk3588` / `rk35xx`。默认值：`stable`。若同时设置了 `kernel_config` 参数，最终优先采用 `config_flavor` 指定的配置文件。这两个参数互斥，可任选其一：通过前者使用 [xrcuo/kernel_arm64](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release) 的内核配置文件，或通过后者使用你自己仓库的配置文件。功能参考 `-f` |

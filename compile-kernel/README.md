...
Pre-compiled kernels are available in the [Releases](https://github.com/xrcuo/xrcuo/kernel_arm64/releases) of [xrcuo/kernel_arm64](https://github.com/xrcuo/xrcuo/kernel_arm64).
...
| -f        | configFlavor | Specifies which configuration file `config-*` to download from the kernel repository [xrcuo/kernel_arm64](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release) to the local [tools/config](tools/config)￼ directory. If a configuration file matching the kernel version already exists locally and this parameter is not set, the download is skipped. Available options correspond to the [directory names](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release) in the kernel repository, e.g., `stable` / `rk3588` / `rk35xx`. Default: `stable`. |
...
💡 Tip: We recommend using the kernel source code from repositories such as [linux-6.1.y](https://github.com/unifreq/linux-6.1.y), [linux-5.15.y](https://github.com/unifreq/linux-5.15.y), [linux-5.10.y](https://github.com/unifreq/linux-5.10.y) of `unifreq` for compilation, who added drivers and patches for related boxes. It is recommended to use the template in [xrcuo/kernel_arm64](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release), which has been pre-configured according to the related boxes and can be customized based on this.
...
| config_flavor    | stable        | Specifies which configuration file `config-*` to download from the kernel repository [xrcuo/kernel_arm64](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release) to the local [tools/config](tools/config)￼ directory. If a configuration file matching the kernel version already exists locally and this parameter is not set, the download is skipped. Available options correspond to the [directory names](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release) in the kernel repository, e.g., `stable` / `rk3588` / `rk35xx`. Default: `stable`. If `kernel_config` is also set, the configuration file specified by `config_flavor` takes precedence. These two parameters are mutually exclusive: use one to select kernel configuration files from [xrcuo/kernel_arm64](https://github.com/xrcuo/xrcuo/kernel_arm64/tree/main/kernel-config/release), or the other to use files from your own repository. Refer to `-f` for details. |
